...
/**
 * Grimy optimizer integration (Universe 1 / Helium)
 * - Uses the upstream optimizer from grimy.github.io (src/perks.ts).
 * - Returns [he_left, perks] like upstream; wrapper below adapts to AutoTrimps' expected return (perks object).
 */
function optimizeGrimy(params) {
...
function optimize() {
	if (typeof getPageSetting === 'function' && getPageSetting('useGrimyPerks', 1)) {
		try {
			const res = optimizeGrimy(populatePerkyData());
			if (Array.isArray(res)) return res[1];
			return res;
		} catch (e) {
			if (typeof console !== 'undefined' && console.error) {
				console.error('AutoTrimps: Grimy perk optimizer failed; falling back to built-in optimizer.', e);
			}
		}
	}
	return optimizeLegacy();
}
