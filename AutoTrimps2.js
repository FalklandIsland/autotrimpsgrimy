...
let trimpStats = { isC3: false, isDaily: false, isFiller: false, mountainPriority: false, fluffyRewards: { universe: 0, level: 0 } };/* Auto-detect basepath from the URL this script was loaded from (helps forks + Steam mods.js). */
(function () {
	try {
		if (atConfig && atConfig.initialise && (!atConfig.initialise.basepath || atConfig.initialise.basepath === '')) {
			let src = '';
			...
			if (!src) {
				...
				for (let i = scripts.length - 1; i >= 0; i--) {
					const s = scripts[i];
					if (s && s.src && s.src.indexOf('AutoTrimps2.js') !== -1) { src = s.src; break; }
				}
			}
			if (src) atConfig.initialise.basepath = src.substring(0, src.lastIndexOf('/') + 1);
		}
		if (!atConfig.initialise.basepath) atConfig.initialise.basepath = 'https://SadAugust.github.io/AutoTrimps/';
	} catch (e) {}
})();
